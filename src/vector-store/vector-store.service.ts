...
export class VectorStoreService {
    ...
    try {
      ...
      const results = await this.vectorChunkRepository
        ...
        .select([
          ...
          'chunk.text',
          'chunk.metadata',
          `1 - (chunk.embedding <=> :embedding::vector) as similarity`,
        ])
        .where('chunk.botId = :botId', { botId })
        .andWhere(`1 - (chunk.embedding <=> :embedding::vector) > :threshold`, { 
          embedding: embedding, // Không sử dụng JSON.stringify
  ...
  async saveBatchChunks(chunks: Omit<VectorChunk, 'id' | 'createdAt' | 'updatedAt'>[]): Promise<VectorChunk[]> {
    try {
    ...
    try {
      ...
      const results = chunks.map(chunk => {
        ...
        return {
          ...
          text: chunk.text,
          score,
          metadata: chunk.metadata ? (typeof chunk.metadata === 'string' ? JSON.parse(chunk.metadata) : chunk.metadata) : undefined,
        };
      });
